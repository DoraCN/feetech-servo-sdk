...
#[derive(Parser, Debug)]
#[command(author, version, about = "Move 6-DoF Arm to target position smoothly")]
struct Args {
    /// 串口路径
...
async fn main() -> anyhow::Result<()> {
    ...
    // 3. 定义目标姿态 (单位：度)
    // 请根据你的机械臂实际构型修改这些值，防止撞击
    // 这里设置一个稍微复杂的姿态作为演示
    let target_pose_deg = vec![
        0.0, // ID 1: 底座旋转
        0.0, // ID 2: 肩部
        0.0, // ID 3: 肘部
        0.0, // ID 4: 腕部旋转
    ...
    info!("准备移动到目标姿态: {:?}", target_pose_deg);
    info!("计划耗时: {:.1} 秒 (控制速度)", args.duration);

    // 4. 执行平滑移动
    move_smoothly(&mut bus, &arm_ids, &target_pose_deg, args.duration).await?;

    info!("✅ 已到达目标位置。");
    info!("🔒 机械臂保持锁定状态 (Torque Enabled)。");
    info!("程序结束。");
