...
pub enum ControlOp {
    ...
    ///
    /// 指令舵机移动到特定角度。
    /// - **参数**: 目标位置（单位: 弧度，通常范围为 -π 到 π）。
    Position(f32),
    /// 速度控制模式 (预留功能，暂未对驱动层完全开放)
    ///
    /// 控制舵机的旋转速度。
    /// - **参数**: 目标速度（单位: 弧度/秒）。
    Velocity(f32),
    /// 原始数据控制 (通过原始 Tick 直接驱动)
    ///
    /// 绕过 SDK 的物理单位换算，直接向寄存器写入原始 16 位数值。
    /// 常用于调试或特殊的 0-4095 范围控制。
    /// - **参数**: 原始位置或对应数据 (例如 STS3215 对应 0 ~ 4095)。
    RawEffort(i16),
}
